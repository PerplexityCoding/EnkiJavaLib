...
{
	public static Logger log = LoggerFactory.getLogger(Mustache.class);
    ...
    protected static Template compile (Reader source, Compiler compiler)
    {
        ...
        case MATCHING_END:
            ...
            break;
        case TAG:
            log.error("Template ended while parsing a tag [line=" + line + ", tag="+ text + "]");
            text.append(end1);
            accum.addTextSegment(text);
            break;
        }
        ...
        for (int ii = 0, ll = accum.length(); ii < ll; ii++) {
            if (accum.charAt(ii) == start1) {
                if (start2 == -1 || (ii < ll-1 && accum.charAt(ii+1) == start2)) {
                    log.error("Tag contains start tag delimiter, probably missing close delimiter " +
                            "[line=" + line + ", tag=" + accum + "]");
                    return false;
                }
            }
